...
- **源文联动** — 选中任意差异时，自动在原始文本与修订文本中高亮对应片段，无需手动定位
- **滚动同步** — 左右两个输入框按比例联动滚动，始终对照同一位置
- **点击跳转** — 在差异预览区点击任意差异项即可跳转选中
- **鼠标拖选** — 差异预览区支持鼠标滑选，选中部分蓝色高亮
- **一键粘贴** — 每个输入框提供「粘贴」按钮，快速从剪贴板导入文本
- **Windows 11 外观** — 参照 Fluent Design 抗锯齿自绘圆角按钮与卡片（无毛边/白圈）、Win11 覆盖式滚动条（悬停展开、内容不满时自动隐藏）；顶部紧凑分组工具栏（主操作 → 导航 → 接受/拒绝 → 清空），底部状态栏带快捷键提示，风格贴近系统记事本
- **键盘快捷键** — `Ctrl+Enter` 直接对比，`F1` / `F2` 切换上一处 / 下一处差异，`F3` / `F4` 接受 / 拒绝
- **高 DPI 适配** — 自动适配高分辨率屏幕
...
4. 使用 **「上一项 / 下一项」**（或 `F1` / `F2`）逐个浏览差异；每选中一处，原始/修订文本框会自动滚动并高亮对应片段
5. 对每处差异选择 **「接受」**（采用修订版，或按 `F3`）或 **「拒绝」**（保留原文，或按 `F4`）
6. 处理完毕后，状态栏显示处理结果
